...
function generateWordSearch(words: string[], lang: Language, size = 10): { grid: Grid; targets: string[] } {
  ...
  for (const word of words) {
    ...
    for (let attempt = 0; attempt < 50 && !success; attempt++) {
      ...
      if (!ok) continue;
      for (let i = 0; i < w.length; i++) grid[y + dir.dy * i][x + dir.dx * i] = w[i];
      placed.push(w);
      success = true;
    }
  }
      ...
      <View style={styles.container}>
        ...
        {found.size === targets.length && (
          <View style={{ alignItems: "center", marginTop: 8 }}>
            <FontAwesome5 name="check-circle" size={20} color="#9ee2acff" style={{ opacity: 0.9 }} />
...
const styles = StyleSheet.create({
  ...
  cell: { width: 32, height: 32, margin: 2, backgroundColor: "rgba(255,255,255,0.25)", justifyContent: "center", alignItems: "center", borderRadius: 4 },
  selected: { backgroundColor: "#bfc1c5ff" },
  found: { backgroundColor: "#8fb8a2" },
  letter: { color: "white", fontWeight: "bold" },
  button: { backgroundColor: "rgba(255,255,255,0.35)", paddingHorizontal: 20, paddingVertical: 10, borderRadius: 10, marginTop: 10 },
